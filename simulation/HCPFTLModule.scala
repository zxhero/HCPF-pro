...
class ReadStage(params: HCPFParams) extends Module {
  ...
  }.elsewhen(io.axi.r_last === true.B && io.axi.r_valid === true.B && io.axi.r_ready === true.B && io.axi.r_id(1, 0) === 1.U(2.W) ){
    rd_status1(rd_table_ptr) := 1.U
  }
  when(io.StatusReset2 === true.B){
    rd_status2(io.FreeEntryPtr2) := 0.U(1.W)
  }.elsewhen(io.axi.r_last === true.B && io.axi.r_valid === true.B && io.axi.r_ready === true.B && io.axi.r_id(1, 0) === 2.U(2.W) ){
  ...
  }.elsewhen(io.axi.r_last === true.B && io.axi.r_valid === true.B && io.axi.r_ready === true.B && io.axi.r_id(1, 0) === 3.U(2.W) ){
  ...
  when(send_first_table === true.B) {
    ...
    io.axi.ar_id := Cat((io.rdAddrPtr2 - io.CpuOperatePtr) % 16.U(6.W), 2.U(2.W))
    ...
    io.axi.ar_id := Cat((io.rdAddrPtr3 - io.CpuOperatePtr) % 16.U(6.W), 3.U(2.W))
...
class HCPFType3Controller (params: HCPFParams) extends  Module{
  ...
  when(io.PtrReset === true.B){
    cpu_operate_ptr := 0.U
  }.elsewhen(read_finish === true.B && global_state === true.B){
    cpu_operate_ptr := cpu_operate_ptr + request_queue.io.deq.bits.others(51,48) + 1.U
  }
